...
CREATE TABLE `plan_attraction` (
  ...
  `trip_plan_id` bigint DEFAULT NULL,
  `created_date` datetime(6) DEFAULT NULL,
  `last_modified_date` datetime(6) DEFAULT NULL,
  `plan_order` bigint DEFAULT NULL,
  PRIMARY KEY (`plan_attraction_id`),
  KEY `plan_attraction_fk` (`attraction_id`),
  KEY `plan_trip_fk` (`trip_plan_id`),
  CONSTRAINT `plan_attraction_fk` FOREIGN KEY (`attraction_id`) REFERENCES `attraction_info` (`content_id`),
  CONSTRAINT `plan_trip_fk` FOREIGN KEY (`trip_plan_id`) REFERENCES `trip_plan` (`trip_plan_id`)
